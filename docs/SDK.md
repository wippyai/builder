...
compiles those APIs.
The example pins runtime
[`v0.3.43a`](https://github.com/wippyai/runtime/releases/tag/v0.3.43a).
Its executable launch surface is `cmd/app.Executable`, introduced in runtime
[PR #787](https://github.com/wippyai/runtime/pull/787). Go integrations require
the documented runtime revision.
...
| Command and data paths | `wippy.build.json` `application` | New executable |
...
[runtime configuration contract](https://github.com/wippyai/runtime/blob/fd1741b1d86a26ef1d6a65a50b92526a8554c009/boot/deps/config/config.go)
...
The generated entry point passes selected components to `cmd/app.Main`.
...
cannot install Go code. `recover` boots shipped code with separate registry
history, without replacing the installed selection or rolling back user databases.
