...
`private: true` for a private module. A manifest may select multiple unique
package and factory entries from the same module version. Every factory call
must create its own service state.
